% greedy attacks:
% an adversary use a greedy algorithm to remove robots' trajectories
% greedy removal
function [N_gre_remo_remain]= gre_attack(tar_cover, r_tra_inx)
    ...
    %define gre_attack
    gre_attack_set = cell(1,N_atk + 1);
    %define robot attacked. 
    gre_r_attack = zeros(1,N_atk); 
   
    for r = 1 : N_atk % greedy needs N_failure rounds
        ...
        gre_attack_set{r+1} = union(tar_cover{gre_r_tra_inx(max_inx,1), gre_r_tra_inx(max_inx,2)}, ...
                gre_attack_set{r});
...
%                                                                r_tra_inx(i,2)});
        if ismember(r_tra_inx(i,1), gre_r_attack) == 1
